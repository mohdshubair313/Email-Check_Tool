...
import { sendReportEmail } from '@/app/action'; // Adjust import as per your project
import { Spinner } from "@/components/ui/spinner";
import { toast } from 'sonner';
import jsPDF from 'jspdf';

export default function ReportTableClient({ code }: { code: string }) {
  ...
  useEffect(() => {
    if (code) {
      // Fetch data from DB via action/fetch/api
      setReport({
        code,
        results: ['Inbox', 'Spam', 'Not Received', 'Inbox', 'Promotions'],
        score: 60,
      });
      setLoading(false);
    } else {
  ...
  const shareLink = typeof window !== 'undefined'
    ? `${window.location.origin}/report?code=${code}`
    : '';
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-8 max-w-4xl mx-auto">
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
        <button onClick={() => navigator.clipboard.writeText(shareLink)} className="bg-blue-500 text-white p-3 rounded hover:bg-blue-600">
          Copy Share Link
        </button>
        <div className="md:col-span-2">
          <input type="email" value={userEmail} onChange={(e) => setUserEmail(e.target.value)} placeholder="Your email" className="border p-2 rounded w-full md:w-1/2" />
          <button
